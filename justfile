...
#   - an empty or whitespace-only translation ("key": "") in any ARB:
#     gen-l10n counts it as translated and generates a getter that returns
#     it — a blank label at runtime. Only top-level string values count; an
#     empty field inside an "@key" metadata block is not a translation.
check_l10n: gen_l10n
  #!/usr/bin/env bash
  set -euo pipefail
  ...
  # The untranslated report is written first — to stdout and, on CI, to the
  # job summary — so it is there to read even when the blank-value check
  # below fails the run.
  report="$(python3 - <<'PY'
  import json
  ...
  blank = [
      ...
      if not key.startswith("@") and isinstance(value, str) and not value.strip()
  ]
  if blank:
