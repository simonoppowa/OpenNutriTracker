...
///
/// [root] is the repository root; tests point it at a fixture directory.
LocaleCheck checkLocales({bool fix = false, String root = '.'}) {
  File file(String relative) => File('$root/$relative');
  ...
  for (final arb in Directory('$root/lib/l10n').listSync().whereType<File>()) {
    ...
    arbLocales[code] = _arbKeys(arb).length;
  }
  ...
  if (plistBlock == null) {
    ...
    // Only live entries count; a locale inside an XML comment is not
    // declared to iOS.
    final uncommented = plistBlock
        .group(2)!
  ...
  // Comments are stripped first so a mention in prose cannot satisfy the
  // check; the identifier has to appear in code.
  final settings = file(_settingsPath)
      .readAsStringSync()
      .replaceAll(RegExp(r'//[^\n]*'), '')
      .replaceAll(RegExp(r'/\*.*?\*/', dotAll: true), '');
  ...
  } else if (!RegExp(r'\bshippedLocales\b').hasMatch(settings)) {
