...
**Three providers, and they differ along two axes that do not line up.** Anthropic and OpenAI are reached directly; OpenRouter is a **broker**, so a request touches OpenRouter *and* the vendor serving your chosen model — two destinations, not one. Both curated OpenRouter models are pinned to Anthropic with fallbacks off, so the vendor named in Settings is the vendor that answered; without a pin a request for a Claude model is routinely served by someone else, which is why the pin exists. A key is stored per provider, so switching back does not mean finding a credential again.
