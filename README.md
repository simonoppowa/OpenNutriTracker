...
| [Anthropic](https://www.anthropic.com/) | **Only if you save your own API key** | The meal line you type on the multi-item add screen, or a meal photo you choose to read there, and your app language |
...
**Reading a meal photo** is the same feature with a camera instead of a keyboard, and is offered only once a key is enabled. The photo is encoded in memory and sent for that one request: the app never writes it to disk, so it is not in your exports and cannot be shown again. Anthropic's [vision documentation](https://platform.claude.com/docs/en/build-with-claude/vision) states images are *"ephemeral and not stored beyond the duration of the API request"* and are not used to train models.
