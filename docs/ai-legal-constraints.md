...
> conclusions below. Where they conflict, they win. They are named rather
> than linked: they are working notes that live alongside this one and are
> not part of the repository.
>
> - `ai-cohort-restrictions.md` corrects the
...
>   `fdroid-submission-feasibility.md`,
>   written earlier for #575.)
> - `ai-open-research-questions.md` closes
...
**The "Encryption in transit" declaration is no longer unconditionally
true.** It was, while every endpoint was a compile-time `https://`
constant. [#758](https://github.com/simonoppowa/OpenNutriTracker/issues/758)
let a user type an address, and [#746](https://github.com/simonoppowa/OpenNutriTracker/issues/746)
and [#748](https://github.com/simonoppowa/OpenNutriTracker/issues/748)
measured that nothing in the HTTP stack prevents `http://` on either
platform.

What is true now: the app refuses to send in the clear to anything that is
not a private address, so plaintext is confined to a network the user
controls — but the declaration itself is **app-wide and all-or-nothing**,
with no conditional form and no prominent-disclosure exception, researched
against Play's own form for
[#751](https://github.com/simonoppowa/OpenNutriTracker/issues/751).
So the form cannot express "https except to a private address the user
