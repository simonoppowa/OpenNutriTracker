...
just gen_l10n      # flutter gen-l10n — regenerate localizations from lib/l10n/*.arb
just ci            # full CI: install, format check, l10n generation, build, analyze, test
...
**`lib/core/utils/env.g.dart`** and the `lib/generated/` localization output are gitignored. After a fresh clone, run `just build` with a valid `.env` file and `just gen_l10n` before the app will compile (a plain `flutter run`/`build` also triggers gen-l10n automatically).

## Localization

Source strings live in `lib/l10n/intl_en.arb` (the template; other locale ARBs: `de`, `cs`, `it`, `pl`, `sk`, `tr`, `uk`, `zh`). Dart bindings are generated by Flutter's built-in `gen-l10n` (configured in `l10n.yaml`) into `lib/generated/` — **gitignored build output**, regenerated automatically on `flutter run`/`build` (`generate: true` in pubspec) or manually via `just gen_l10n`. Never edit generated files.

The generated class keeps the legacy name `S` and import path `package:opennutritracker/generated/l10n.dart`, so call sites read `S.of(context).someKey`. Conventions:

- Declare placeholders with types in `@key` metadata in `intl_en.arb` (plural placeholders must be `num`/`int`).
- Keys missing from a locale ARB fall back to the English source at generation time and are listed in `l10n_untranslated.json` (gitignored).
- Outside the widget tree (startup notification bootstrap, tests) use the generated `lookupS(locale)`; wrap device-derived locales in `basicLocaleListResolution([...], S.supportedLocales)` first, because `lookupS` throws on unsupported locales. There is no `S.current`; tests use the `l10nEn` helper from `test/helpers/test_l10n.dart`.
...
lib/
  ...
  generated/      # gen-l10n output — gitignored, never committed (see Localization above)
