...
void main() {
  ...
  setUp(() async {
    tempRoot = await Directory.systemTemp.createTemp('ont_recipe_img_test_');
  ...
  group('RecipeImageStorage', () {
    ...
    test(
      ...
      // Source file must still exist — importFrom copies, never moves.
    ...
    test('delete removes the file at the relative slug and tolerates '
      ...
      await RecipeImageStorage.delete(relative);
      final absolute = await RecipeImageStorage.absolutePath(relative);
      expect(await File(absolute).exists(), isFalse);

      // Second delete is a no-op — callers shouldn't have to guard against
      // a stale path that has already been cleaned up.
      await RecipeImageStorage.delete(relative);
    });
    ...
    test('full round-trip: store, resolve, delete', () async {
      ...
      final relative = await RecipeImageStorage.importFrom(
        recipeId: 'round-trip-id',
        sourcePath: source.path,
      );

      // The path stored on the DBO is intentionally relative so that an
